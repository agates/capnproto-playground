...
class AtlasI2c:
    ...
    def read_binary(self, num_of_bytes=31):
        ...
        reponse_code = response.__next__()
        if reponse_code == 1:
            ...
            return "Error " + str(reponse_code)

    def read(self, num_of_bytes=31):
        return ''.join(chr(x) for x in self.read_binary(num_of_bytes))
...
def main():
    ...
    while True:
        ...
        # if not a special keyword, pass commands straight to board
            ...
            try:
                temp_response = device.query(myinput)
                print(temp_response)
